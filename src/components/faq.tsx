...
const FAQItemComponent: React.FC<FAQItemComponentProps> = React.memo(({ item, index, isOpen, onToggle }) => {
      ...
      >
        <div className="flex items-center space-x-4 flex-1">
          <div className="flex-shrink-0">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-lg ${categoryStyles.bg} ${categoryStyles.text}`}>
              {categoryStyles.icon}
            </div>
          </div>
...
const CategoryButton: React.FC<CategoryButtonProps> = React.memo(({ category, isActive, count, onClick }) => {
  const handleClick = useCallback(() => {
    onClick(category);
  }, [category, onClick]);
...
const FAQ: React.FC = React.memo(() => {
  const [activeCategory, setActiveCategory] = useState<'all' | 'game' | 'technical' | 'strategy'>('all');
  ...
  const faqData: FAQItem[] = useMemo(() => [
    // Game-related questions
    {
      ...
      answer: 'Waffle is a word swap puzzle game. Players need to form 6 valid English words (3 horizontal, 3 vertical) in a 5x5 grid by swapping letter positions. The game provides color hints: green indicates correct position, yellow indicates the letter exists but is in wrong position, gray indicates the letter does not belong to that word.',
      category: 'game'
    },
    {
      question: 'How do I know when the game is completed?',
      answer: 'When all 21 letter tiles turn green, it means all 6 words have been correctly formed and the game is complete. The system will automatically detect and display victory information.',
      category: 'game'
    },
    {
      question: 'Is there a move limit for each puzzle?',
      answer: 'Yes, each Waffle puzzle has a maximum move limit, typically 15 moves. This adds challenge to the game, requiring players to think carefully about each swap.',
      category: 'game'
    },
    {
      question: 'How does the AI suggestion feature work?',
      answer: 'The AI suggestion feature analyzes the current board state, calculates all possible swap combinations, and recommends optimal swap strategies that maximize the number of correct letters. This helps players get hints when stuck.',
      category: 'game'
    },
    {
      question: 'Can I generate custom themed puzzles?',
      answer: 'Yes, the project supports AI generation functionality that can create word puzzles based on different themes (such as nature, technology, emotions, etc.). Simply select a theme and use the AI generator.',
      category: 'game'
    },
    ...
    // Strategy-related questions
    {
      ...
      answer: 'Recommended strategies include: 1) Prioritize fixing green letters as anchors; 2) Analyze possible positions for yellow letters; 3) Identify common English word patterns; 4) Start from intersection points, as these letters belong to two words simultaneously; 5) Use AI suggestions wisely.',
      category: 'strategy'
    },
    {
      question: 'How to handle complex swap situations?',
      answer: 'For complex situations, consider: 1) Solve partial words first to establish a foundation; 2) Utilize word intersection characteristics, as one swap may affect multiple words; 3) Use elimination method to determine which letters don\'t belong to specific positions; 4) Use AI suggestions when necessary for optimal solutions.',
      category: 'strategy'
    },
    {
      question: 'How to improve solving efficiency?',
      answer: 'Methods to improve efficiency include: 1) Familiarize yourself with common English word patterns; 2) Prioritize areas with more clues; 3) Learn to identify word roots, prefixes, and suffixes; 4) Practice spatial thinking to predict swap results; 5) Allocate moves wisely.',
      category: 'strategy'
    },
    ...
    // Technical questions
    {
      ...
      answer: 'The project is built on Next.js 15, using React 18, TypeScript, and Tailwind CSS for development. The UI component library uses shadcn/ui, state management uses React Hooks, and integrates AI functionality and animation systems.',
      category: 'technical'
    },
    {
    ...
    },
    {
      ...
      answer: 'Performance optimization measures include: 1) Using React.memo and useMemo to reduce unnecessary re-renders; 2) Implementing virtualization and lazy loading; 3) Optimizing algorithm complexity; 4) Using Web Workers for complex calculations; 5) Using CSS animations instead of JavaScript animations for better smoothness.',
      category: 'technical'
    },
    {
      question: 'Which browsers does the project support?',
      answer: 'The project supports all modern browsers, including Chrome 90+, Firefox 88+, Safari 14+, Edge 90+. It uses modern web standards to ensure good experience across different devices and browsers.',
      category: 'technical'
    }
  ], []);
  ...
  const handleCategoryChange = useCallback((category: 'all' | 'game' | 'technical' | 'strategy') => {
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-16">
        ...
        <div className="mb-12">
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-6 border border-slate-200/80 shadow-lg">
            <h3 className="text-xl font-semibold text-slate-800 mb-6 text-center">Question Categories</h3>
            <div className="flex flex-wrap justify-center gap-3">
              {categories.map((category) => (
                <CategoryButton
                  ...
                  count={categoryCounts[category.key as keyof typeof categoryCounts]}
        ...
        {filteredFAQ.length === 0 && (
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold text-slate-800 mb-2">No Related Questions</h3>
            <p className="text-slate-700">Please select other categories or view all questions</p>
          </div>
        )}
          ...
          <div className="text-center">
            <div className="bg-gradient-to-r from-slate-100/80 to-blue-100/80 backdrop-blur-sm rounded-xl p-6 border border-slate-300/60 shadow-md">
              <p className="text-slate-700 font-medium">
                💫 Can't find the answer you're looking for? Try using the "Get Suggestion" feature in the game, or generate new puzzle challenges!
              </p>
            </div>
          </div>
        </div>
