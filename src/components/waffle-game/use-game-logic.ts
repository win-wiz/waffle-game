...
import { createWaffleSolution, WaffleSolution, generateGuaranteedWaffle } from '@/lib/words';
import {
...
export function useGameLogic() {
  ...
  const [currentSolution, setCurrentSolution] = useState<WaffleSolution | null>(null);
  ...
  const [currentSuggestion, setCurrentSuggestion] = useState<{ from: SquareNumber; to: SquareNumber } | null>(null);
  ...
  const [validClassicPuzzles, setValidClassicPuzzles] = useState<typeof classicPuzzles>([]);

  // Anti-duplicate toast state
  const [lastToastTime, setLastToastTime] = useState<number>(0);

  // Puzzle type tracking for statistics
  const [currentPuzzleType, setCurrentPuzzleType] = useState<'random' | 'classic' | 'ai-generated'>('random');
  ...
  const showToastWithDeduplication = useCallback((toastFn: Function, message: string, options?: any, deduplicationTime: number = 2000) => {
    const now = Date.now();
  ...
  const recalculateColors = useCallback((currentBoard: Board<Tile>): Board<Tile> => {
    const targetBoard = currentSolution?.board || SOLUTION_BOARD;
    return currentBoard.map((tile, index) => {
      const newColor = getColorOfSquare(tile.letter, targetBoard as Board<Letter>, index as SquareNumber);
      return {
        ...tile,
        color: newColor
      };
  ...
  const handleTileClick = useCallback((position: SquareNumber) => {
    ...
    if (currentTile?.color === Color.Green) {
      ...
      console.log(`Letter "${currentTile.letter}" at position ${position} is already in correct position, cannot swap`);
      return;
    }
  ...
  // Perform letter swap
  const performSwap = useCallback((pos1: SquareNumber, pos2: SquareNumber) => {
    if (!board) return;
    if (pos1 === pos2) return;
    ...
    console.log(`📋 Tile info: ${pos1}(${tile1?.letter}, ${tile1?.color}) ↔ ${pos2}(${tile2?.letter}, ${tile2?.color})`);

    if (tile1?.color === Color.Green) {
      console.log(`❌ Letter "${tile1.letter}" at position ${pos1} is already in correct position, cannot swap`);
    ...
    if (tile2?.color === Color.Green) {
      console.log(`❌ Letter "${tile2.letter}" at position ${pos2} is already in correct position, cannot swap`);
      toast.error('⚠️ Cannot Swap', {
    ...
    console.log(`🚀 [performSwap] Setting swap animation: ${pos1}(${tile1?.letter}) ↔ ${pos2}(${tile2?.letter})`);
    ...
    console.log(`📍 [performSwap] Set animation positions: [${pos1}, ${pos2}]`);
    ...
    setTimeout(() => {
      ...
      console.log('   - animatingPositions current value:', animatingPositions);
  ...
  const handleSwapAnimationComplete = useCallback(() => {
    ...
    console.log('🔄 [handleSwapAnimationComplete] Current swapAnimation:', swapAnimation);

    if (!swapAnimation) {
      console.log('⚠️ [handleSwapAnimationComplete] swapAnimation is null, exiting');
      return;
    }

    const { from, to } = swapAnimation;
    console.log(`🔄 [handleSwapAnimationComplete] Executing swap: ${from}(${swapAnimation.fromLetter}) ↔ ${to}(${swapAnimation.toLetter})`);

    // Perform actual letter swap
    const newBoard = [...board] as Board<Tile>;
    console.log('🔄 [handleSwapAnimationComplete] Board before swap:', newBoard.map((tile, i) => `${i}:${tile.letter}`));

    const temp = newBoard[from].letter;
    newBoard[from] = { ...newBoard[from], letter: newBoard[to].letter };
    newBoard[to] = { ...newBoard[to], letter: temp };

    console.log('🔄 [handleSwapAnimationComplete] Board after swap:', newBoard.map((tile, i) => `${i}:${tile.letter}`));

    // Recalculate colors for all positions
    const boardWithNewColors = recalculateColors(newBoard);
    console.log('🔄 [handleSwapAnimationComplete] After color recalculation:', boardWithNewColors.map((tile, i) => `${i}:${tile.letter}(${tile.color})`));
    ...
    const isCompleted = boardWithNewColors.every(tile => tile.color === Color.Green);
    ...
    console.log(`✅ [handleSwapAnimationComplete] Swap completed: position ${from} ↔ position ${to}`);
  ...
  const getSuggestion = useCallback(async () => {
    ...
    try {
      ...
      console.log('Current board state:', board.map(tile => `${tile.letter}(${tile.color})`));

      // Analyze current game state
      const possibleSolutions = possibleSolutionsFromBoard(board && board.length === 21 ? board : Array(21).fill({letter: '', color: Color.Grey}) as Board<Tile>);
        ...
        if (wrongPositions.length >= 2) {
          ...
          for (let i = 0; i < wrongPositions.length; i++) {
            for (let j = i + 1; j < wrongPositions.length; j++) {
              ...
              if (pos1.tile.letter === pos2.targetLetter || pos2.tile.letter === pos1.targetLetter) {
          ...
          setSolution(null);
          setSwaps([[bestSwap[0] as SquareNumber, bestSwap[1] as SquareNumber]]);

          toast.info('💡 Smart Suggestion', {
            description: `Suggest swapping ${getIntuitivePosDescription(bestSwap[0] as SquareNumber, board && board.length === 21 ? board : Array(21).fill({letter: '', color: Color.Grey}) as Board<Tile>)} and ${getIntuitivePosDescription(bestSwap[1] as SquareNumber, board && board.length === 21 ? board : Array(21).fill({letter: '', color: Color.Grey}) as Board<Tile>)}, this might improve current situation.`
          });
          return;
        }

        toast.error('Current state cannot find valid solution, suggest restarting the game.');
        return;
      }

      const startString = transformBoard(board && board.length === 21 ? board : Array(21).fill({letter: '', color: Color.Grey}) as Board<Tile>, (tile) => tile.letter).join('');
      ...
      if (possibleSolutions.size === 1) {
        ...
        toast.info('💡 AI Suggestion', {
          description: `Suggest swapping ${getIntuitivePosDescription(bestSwap[0], board && board.length === 21 ? board : Array(21).fill({letter: '', color: Color.Grey}) as Board<Tile>)} and ${getIntuitivePosDescription(bestSwap[1], board && board.length === 21 ? board : Array(21).fill({letter: '', color: Color.Grey}) as Board<Tile>)}, this is the optimal next step.`
        });
      }
    } catch (error) {
      console.error('Failed to get suggestion:', error);
      toast.error('Error occurred during AI analysis, please retry or restart the game.');
    } finally {
      setIsLoading(false);
    }
  ...
  const applySuggestedSwap = useCallback(() => {
    ...
    toast.info('✨ Apply AI Suggestion', {
      description: `Swapping ${getIntuitivePosDescription(from, board && board.length === 21 ? board : Array(21).fill({letter: '', color: Color.Grey}) as Board<Tile>)} and ${getIntuitivePosDescription(to, board && board.length === 21 ? board : Array(21).fill({letter: '', color: Color.Grey}) as Board<Tile>)}`
    });
  ...
  const generateNewPuzzle = useCallback(async (showSuccessToast: boolean = true) => {
    ...
    try {
      ...
      if (showSuccessToast) {
        showToastWithDeduplication(toast.success, '🎯 Brand new Waffle puzzle is ready, start the challenge!');
      }
    } catch (error) {
      console.error('Failed to generate puzzle:', error);
      toast.error('Puzzle generation failed, will use classic puzzle as fallback');
      // If it really fails, use classic puzzle as fallback
      try {
        const classicSolution = createWaffleSolution('SIREN', 'RABBI', 'WINCH', 'SCREW', 'ROBIN', 'NEIGH');
        if (classicSolution) {
        ...
        toast.error('Error occurred while generating puzzle, please refresh the page and try again');
      }
    } finally {
  ...
  const useClassicPuzzle = useCallback(async () => {
    ...
    try {
      ...
      const availablePuzzles = validClassicPuzzles.length > 0 ? validClassicPuzzles : classicPuzzles;
  ...
  const handleValidateAIResponse = useCallback(() => {
    ...
    const validation = validateAIGeneratedWords(parsed.horizontal_words, parsed.vertical_words);

    if (validation.isValid) {
      const solution = createWaffleSolution(
        parsed.horizontal_words[0], parsed.horizontal_words[1], parsed.horizontal_words[2],
        parsed.vertical_words[0], parsed.vertical_words[1], parsed.vertical_words[2]
      );
      ...
      if (solution) {
        ...
        toast.success('✅ AI generated puzzle validated successfully and applied!');
      } else {
        toast.error('❌ Validation failed: Unable to create valid Waffle solution');
      }
    } else {
  ...
  const validateAndFilterPuzzles = useCallback(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`🎯 Classic puzzle validation completed: ${validPuzzles.length}/${classicPuzzles.length} valid`);
    }
  ...
  const movesRemaining = useMemo(() => Math.max(0, maxMoves - moveCount), [maxMoves, moveCount]);
  ...
  const gameState = useMemo(() => ({
    ...
    currentSuggestion,
    showGameEndDialog
  }), [board, solution, swaps, isLoading, selectedPositions, moveCount, movesRemaining, gameStatus, maxMoves, isLoadingSuggestion, currentSuggestion, showGameEndDialog]);
  ...
  const gameOperations = useMemo(() => ({
    ...
    resetGame,
    useClassicPuzzle
  }), [handleTileClick, handleSwapAnimationComplete, getSuggestion, applySuggestedSwap, generateNewPuzzle, resetGame, useClassicPuzzle]);
