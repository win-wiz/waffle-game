...
export const GameControls = memo(function GameControls({
  ...
  const suggestionButtonClass = useMemo(() => {
    const baseClass = 'flex items-center justify-center px-2 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm font-semibold rounded-lg sm:rounded-xl transition-all duration-200';
  ...
  const randomPuzzleButtonClass = useMemo(() => {
    const baseClass = 'flex flex-col items-center justify-center px-2 sm:px-3 py-2 sm:py-3 rounded-lg sm:rounded-xl font-medium transition-all duration-200 text-xs sm:text-sm min-h-[48px] sm:min-h-[56px]';
    const disabledClass = 'bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200';
  ...
  return (
    <div className="w-full max-w-sm sm:max-w-md mx-auto mt-4 sm:mt-6">
      {/* 2x2 Grid Layout */}
      <div className="grid grid-cols-2 gap-2 sm:gap-3">
        ...
        >
          <span className="mr-1 sm:mr-2">{suggestionButtonContent.icon}</span>
          <span className="hidden xs:inline">{suggestionButtonContent.text}</span>
          <span className="xs:hidden">Hint ({remainingHints})</span>
        </button>

        {/* Reset Game */}
        <button
          className="flex items-center justify-center px-2 sm:px-4 py-2 sm:py-3 rounded-lg sm:rounded-xl font-semibold border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 active:scale-95 transition-all duration-200 shadow-sm text-xs sm:text-sm"
          onClick={onResetGame}
        >
          <span className="mr-1 sm:mr-2">🔄</span>
          <span className="hidden xs:inline">Reset Game</span>
        ...
        >
          <span className="text-sm sm:text-base mb-0.5 sm:mb-1">🎲</span>
          <span className="text-xs leading-tight">{randomPuzzleButtonContent.text}</span>
        </button>
        ...
        >
          <span className="text-sm sm:text-base mb-0.5 sm:mb-1">🏛️</span>
          <span className="text-xs leading-tight">{classicPuzzleButtonContent.text}</span>
        </button>
      </div>
