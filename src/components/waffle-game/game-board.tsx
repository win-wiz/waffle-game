...
export function GameBoard({
  ...
  return (
    <div className="w-full flex justify-center items-center">
      <div className="game-board grid grid-cols-5 gap-2 sm:gap-3 md:gap-4 w-fit max-w-sm sm:max-w-md md:max-w-lg mx-auto relative z-10">

        {swapAnimation && (() => {
          if (process.env.NODE_ENV === 'development') {
            console.log('🎬 [GameBoard] 渲染 SwapAnimation 组件:', swapAnimation);
          }
          return (
            <SwapAnimation
        ...
        {Array.from({ length: 25 }, (_, gridIndex) => {
          ...
          if (isEmptyPosition) {
            return <div key={gridIndex} className="dynamic-waffle-tile" style={{ opacity: 0, pointerEvents: 'none' }}></div>;
          }
          ...
          for (let i = 0; i < gridIndex; i++) {
            ...
            const c = i % 5;
            const isEmpty = (r === 1 && (c === 1 || c === 3)) || (r === 3 && (c === 1 || c === 3));
          ...
          if (isLoading || !board) {
            ...
            return (
              <button
                key={gridIndex}
                className={`dynamic-waffle-tile ${isCenterTile ? 'loading-center' : 'loading-empty'}`}
                disabled
              >
                {isCenterTile ? (
          ...
          if (!board || letterIndex >= board.length) {
            return (
              <div key={gridIndex} className="dynamic-waffle-tile" style={{
          ...
          const isSelected = selectedPositions.includes(letterIndex as SquareNumber);
          const isAnimating = animatingPositions.includes(letterIndex as SquareNumber);
          const justSwapped = justSwappedPositions.includes(letterIndex as SquareNumber);
